...
export async function GET() {
  ...
  staticRoutes.forEach(route => {
    xml += '  <url>\n'
    xml += `    <loc>${baseUrl}/zh-CN${route}</loc>\n`
  ...
  serviceCategories.forEach(category => {
    xml += '  <url>\n'
    xml += `    <loc>${baseUrl}/zh-CN/services/${category}</loc>\n`
    xml += `    <lastmod>${new Date().toISOString()}</lastmod>\n`
    xml += `    <changefreq>monthly</changefreq>\n`
  ...
  try {
    ...
    if (newsArticles && newsArticles.length > 0) {
      newsArticles.forEach((article: { slug: string; _updatedAt: string }) => {
        if (article.slug) {
          xml += '  <url>\n'
          xml += `    <loc>${baseUrl}/zh-CN/news/${article.slug}</loc>\n`
    ...
    console.error('Failed to fetch news articles for Chinese (Simplified) sitemap:', error)
  }
