...
// Sanityからカテゴリ一覧を取得
async function getServiceCategories(locale: Locale): Promise<ServiceCategory[]> {
  ...
  try {
    return await sanityClient.fetch(allServiceCategoriesQuery, { locale });
  } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-sm text-gray-600 mt-4 text-center">
            {lang === 'ja' 
              ...
              : '※These are basic fees. Please contact us for details.'}
          </p>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl font-bold text-gray-900 mb-4">
            {lang === 'ja' ? 'まずは無料相談から' : 'Start with a Free Consultation'}
          </h2>
          <p className="text-lg text-gray-600 mb-8">
            {lang === 'ja' 
              ? 'どのようなことでも、お気軽にご相談ください。'
              : 'Feel free to consult with us about anything.'}
          </p>
          <Link
            href={`/${lang}/contact`}
            className="bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors"
          >
            {lang === 'ja' ? '無料相談のお申込み' : 'Apply for Free Consultation'}
          </Link>
        </div>
      </section>
